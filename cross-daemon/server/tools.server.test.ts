...
const newWatches = () => createWatchList(mkdtempSync(join(tmpdir(), "cd-watch-")));
...
describe("list_daemons", () => {
  it("lists the reachable daemons by name and server ID, never by link", async () => {
    const tools = createTools({ readPeers: () => [mac], cli: unusedCli, queue: newQueue(), watches: newWatches(), ownDaemon });
  ...
  it("explains how to connect a daemon when none are reachable", async () => {
    const tools = createTools({ readPeers: () => [], cli: unusedCli, queue: newQueue(), watches: newWatches(), ownDaemon });
    const result = await tools.call("list_daemons", {}, { callerAgentId: "agent-1" });
